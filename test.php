...
<?php $bdd = new PDO('mysql:host=localhost;dbname=projetm1','root','');
                        ...
                        $option_valide = $donne[id_option];
                      }
                        //$option_valide  = $op ["id_option"];
      ?>          
</div>

        <div id="nb-options" style="display: none;">
<?php $bdd = new PDO('mysql:host=localhost;dbname=projetm1','root','');
												$reponse = $bdd->prepare('select nom,id_option from options id_option <> :opt_valide');
                        $reponse->execute(array(
...
<?php
                      try
                      {
                        ...
                        $repons = $bdd->query('select nom,id_option from options WHERE `numero_option` = "";');
                        ...
                        $j++;
                        print ('<td><select id="liste_op'.$i.'.'.$j.'" name="liste_op'.$i.'.'.$j.'" class="form-control" onChange="recupVal('.$i.'.'.$j.')" >');
                        $repons = $bdd->query('select nom,id_option from options WHERE `numero_option` = 1;');
                        echo'<option value="-1" selected="selected"> </option>';
                        while($donne = $repons->fetch()){
                          echo"<option value='$donne[id_option]'>$donne[nom]</option>"; 
                        ...
                        $j++;
                        print ('<td><select id="liste_op'.$i.'.'.$j.'" name="liste_op'.$i.'.'.$j.'" class="form-control" onChange="recupVal('.$i.'.'.$j.')" >');
                        $repons = $bdd->query('select nom,id_option from options WHERE `numero_option` = 2;');
                        echo'<option value="-1" selected="selected"> </option>';
                        while($donne = $repons->fetch()){
                          echo"<option value='$donne[id_option]'>$donne[nom]</option>"; 
