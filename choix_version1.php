...
<?php $bdd = new PDO('mysql:host=localhost;dbname=projetm1','root','');
												$reponse = $bdd->query('select nom,id_option from options');
												$count = $reponse->rowCount();
												echo htmlspecialchars($count);
			?>
...
function recupVal(id){
		hidden_drop(id);
		var suiv = id + 1;
		var ListeOption = document.getElementById('liste_op'+id);
		//ListeOption.disabled="true";
		var elSelSuiv = document.getElementById('liste_op'+suiv);	
		elSelSuiv.innerHTML = ''; 
		...
		for(var i = 0 ;i< l;i++ ){
					...
					else*/ if(!ListeOption.options[i].selected && !false){
						Selection.push(ListeOption.options[i].value,ListeOption.options[i].text);
...
function hidden_drop(id){
		...
		for(var i = next ;i<= nboption;i++ ){
			document.getElementById('liste_op'+i).style.visibility='hidden';
		}		
                ...
                <div class="x_panel">
                    ...
                    <?php
											try
											{
												...
												$i=1;
												while ($i<= $count){ 
												print ("<tr><td><label id='label_".$i."'>Choix $i</label></td>");
                                                print ('<td><select id="liste_op'.$i.'" name="liste_op'.$i.'" class="form-control" onChange="recupVal('.$i.')" >');
												$repons = $bdd->query('select nom,id_option from options');
                                     ...
                                     <button class="btn btn-default submit" type="submit" name="submit" onClick="return verif_ins()" >Accéder</button>
                                     <button class="btn btn-default reset" type="reset" name="reset" onClick="hidden_drop(0);" >Reset</button>
				</div>
