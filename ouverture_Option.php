...
<head>
  ...
  <title><?php echo $_SESSION["identifiant"]; ?></title>
...
<body class="nav-md">
                ...
                <?php
                  try
                  {
                    ...
                    while($donne = $reponse->fetch()){
                      print('<li><a href=resultat_affectation.php?id='.$donne['id_option'].'&nom='.$donne['nom'].'>'.$donne['nom'].'</a></li>');
                    }
                  }
                  catch (Exception $e)
                  {
    ...
    ?>
    <!--affichage la listes des options à ovrir --> 
    ...
    <div class="page-title">
        ...
        if (isset($_POST["ID"])) {  
          ...
          for ($i=0; $i < sizeof($_POST["ID"])  ; $i++) { 
            //echo $_POST["Sites"][$i]."<br/>";
            print ("<tr>");
            print('<td>'.$_POST["nomOption"].'</td>');
            print('<td>'.$_POST["siteOption"].'</td>');
            print('<td>'.$_POST["maxEtudiant"].'</td>');
            print('<td>'.$_POST["ID"][$i].'</td>');
            print ("</tr>");

          }?>
          </tbody>
          </table>
           <form action="envoiMail.php" method="GET" name="">
          <p><b><h3>Choisir les dates convenables svp !</h3></b></p>
          <p> Date début Inscription :
            <input type="date" name="dateDebInsc" placeholder="date début Inscription"  required/>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              Date fin Inscription :
            <input type="date" name="dateFinInsc" placeholder="date fin Inscription" required/>

          </p>
          
          <?php
          for ($i=0; $i < sizeof($_POST["ID"]) ; $i++) { 
            print(" <input type='hidden' name='id".$i."' value='".$_POST['ID'][$i]."' />");
          }
          ?>

            <br/><br/>
            <input type="submit" name="confirmation" value="Confirmer" class="btn btn-info" onclick="confirmation();" />
            <div id="afficheMsg"> </div>
         </form>
          <?php
